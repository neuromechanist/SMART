'''
This module has the same function as `SMART_oneSample_parallel.py`, but I try to keep it in sections, so running the module will be easy in VS Code.
Seyed Yahya Shirazi 09-26-20
...
# %% load data and pre_allocate variables
...
AllResults = [[] for _ in range(nPP)]
for i in range(nPP):
    AllResults[i] = runSmoothing.remote(i)
for i in range(nPP):
    AllResults[i] = ray.get(AllResults[i])
